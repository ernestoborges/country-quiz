...
export default function Quiz() {
    ...
    function handleReset() {
        ...
        fetchQuizData().then(data => { if (data) setQuizData(data); });
    }
    ...
    return (
        <div className="flex flex-col items-center justify-center gap-4 bg-gray-800 p-8 rounded shadow w-full h-[100vh] md:h-full max-w-[400px]">
            ...
            <div>
                <h2 className="text-2xl text-white">Qual país corresponde a esta bandeira?</h2>
            </div>
            <div>
                <div className="bg-white p-1 rounded">
                   <Image src={`https://flagcdn.com/${quizData.country.toLowerCase()}.svg`} alt={`${quizData.country} Flag`} width={200} height={100} />
                </div>
            </div>
            {
                answer !== null && (
            ...
            <div className="w-full">
                <ul className="flex flex-col gap-2" style={{ pointerEvents: isSubmitting ? 'none' : 'auto' }}>
                {
    ...
    return (
        <div className="w-full">
            ...
            >
                {
                    isCorrectAnswer 
                        ? <p className="text-green-700">Correto! A resposta é <strong>{correctAnswer}</strong>.</p>
                        : <p className="text-red-700">Errado! A resposta correta é <strong>{correctAnswer}</strong>.</p>
                }
            </div>
        </div>
    )
